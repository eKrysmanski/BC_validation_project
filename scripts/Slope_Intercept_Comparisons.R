...
full <- ggplot(data = data_full, aes(x = CPM_mean_UN, y = CPM_mean_E)) +
...
TOST_int_full <- tsum_TOST(m1 = 1.19909, 
                           sd1 = 0.0195*sqrt(437), 
                           n1 = 437, 
                           eqb = 0.1, 
                           eqbound_type = "raw",
...
male <- ggplot(data = male_data, aes(x = CPM_mean_UN13, y = CPM_mean_E13)) +
  geom_point(alpha = 0.5) +
  scale_y_log10(breaks = 10^(-1:6),
                labels = scales::label_parse()(paste0("10^", -1:6))) +
  scale_x_log10(breaks = 10^(-1:5),
...
TOST_int_male <- tsum_TOST(m1 = 0.98942, 
                           sd1 = 0.02456*sqrt(n_male), 
                           n1 = n_male, 
                           eqb = 0.1, 
                           eqbound_type = "raw",
                           var.equal = FALSE, 
...
female <- ggplot(data = female_data, aes(x = CPM_mean_UN46, y = CPM_mean_E46)) +
  geom_point(alpha = 0.5) +
  scale_y_log10(breaks = 10^(-1:6),
                labels = scales::label_parse()(paste0("10^", -1:6))) +
  scale_x_log10(breaks = 10^(-1:5),
...
TOST_int_fem <- tsum_TOST(m1 = 1.32267, 
                           sd1 = .01554*sqrt(n_fem), 
                           n1 = n_fem, 
                           eqb = 0.1, 
                           eqbound_type = "raw",
                           var.equal = FALSE, 
