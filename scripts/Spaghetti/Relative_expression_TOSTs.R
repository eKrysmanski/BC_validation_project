...
#Looks like best reference gene here is akr1a1b; lowest M-value and low CV rank
################################################################################
...
#Look at the linear model for the RE normalized data.. 
full <- ggplot(data = FC_data, aes(x = mean_FC_un, y = mean_FC_e)) +
...
full_int_tost <- tsum_TOST(m1 = full_lm_res$coefficients["(Intercept)","Estimate"], 
                          sd1 = full_lm_res$coefficients["(Intercept)", "Std. Error"]*sqrt(nrow(FC_data)), 
                          n1 = nrow(FC_data), 
...
male <- ggplot(data = FC_male, aes(x = mean_FC_un, y = mean_FC_e)) +
  geom_point(alpha = 0.5) +
...
male_int_tost <- tsum_TOST(m1 = male_lm_res$coefficients["(Intercept)","Estimate"], 
                       sd1 = male_lm_res$coefficients["(Intercept)", "Std. Error"]*sqrt(nrow(FC_male)), 
                       n1 = nrow(FC_male), 
                       eqb = 0.1, eqbound_type = "raw", var.equal = FALSE)

#Slope TOST
male_slope_tost <- tsum_TOST(m1 = male_lm_res$coefficients["log10(mean_FC_un)","Estimate"] - 1, 
                             sd1 = male_lm_res$coefficients["log10(mean_FC_un)", "Std. Error"]*sqrt(nrow(FC_male)), 
                             n1 = nrow(FC_male), 
                             mu = 1,
                             eqb = 0.1, eqbound_type = "raw", var.equal = FALSE)
...
#plot to take a look; looks pretty good
fem <- ggplot(data = FC_fem, aes(x = mean_FC_un, y = mean_FC_e)) +
  geom_point(alpha = 0.5) +
...
#Create a simple figure to plot the TOST results for slope and intercepts
...
full_int_plot_data <- data.frame(
  d_intercept = full_int_tost$effsize["Raw", "estimate"], 
  CI_lower = full_int_tost$effsize["Raw", "lower.ci"], 
  CI_upper = full_int_tost$effsize["Raw", "upper.ci"])

male_int_plot_data <- data.frame(
  d_intercept = male_int_tost$effsize["Raw", "estimate"], 
  CI_lower = male_int_tost$effsize["Raw", "lower.ci"], 
  CI_upper = male_int_tost$effsize["Raw", "upper.ci"])

fem_int_plot_data <- data.frame(
  d_intercept = fem_int_tost$effsize["Raw", "estimate"], 
  CI_lower = fem_int_tost$effsize["Raw", "lower.ci"], 
  CI_upper = fem_int_tost$effsize["Raw", "upper.ci"])


ggplot() +
  geom_vline(aes(xintercept = 0.1), linetype = "dashed", color = "black", alpha = 0.5) +
  geom_vline(aes(xintercept = -0.1),  linetype = "dashed", color = "black", alpha = 0.5) +
  geom_vline(aes(xintercept = 0), linetype = "dashed", color = "indianred", alpha = 0.5) +
  geom_point(data = fem_int_plot_data, 
             aes(y = 1, x = d_intercept), 
             size = 5, 
             shape = 23, 
             colour = "indianred", 
             fill = "indianred") +
  geom_errorbar(data = fem_int_plot_data, 
                aes(y = 1,
                    x = d_intercept,
                    xmin = CI_lower,
                    xmax = CI_upper), 
                ...
                alpha = 0.5,
                colour = "indianred") +
  geom_text(data = fem_int_plot_data, 
            aes(x = d_intercept, y = 1, label = "Female Data"), 
            size = 4, colour = "indianred", 
            vjust = -2.5) +
  geom_point(data = male_int_plot_data,
             aes(y = 0.75, x = d_intercept), 
             size = 5, 
             shape = 23, 
             colour = "navy", 
             fill = "darkblue") +
  geom_errorbar(data = male_int_plot_data, 
                aes(y = 0.75, x = d_intercept, 
                    xmin = CI_lower,
                    xmax = CI_upper), 
                width = 0, 
                ...
                alpha = 0.5,
                colour = "navy") + 
  geom_text(data = male_int_plot_data, 
            aes(x = d_intercept, y = 0.75, label = "Male Data"), 
            size = 4, colour = "navy", 
            vjust = -2.5) +
  
  geom_point(data = full_int_plot_data,
             aes(y = 1.25, x = d_intercept), 
             size = 5, 
             shape = 23, 
             colour = "black", 
             fill = "black") +
  geom_errorbar(data = full_int_plot_data, 
                aes(y = 1.25, x = d_intercept, 
                    xmin = CI_lower,
                    xmax = CI_upper), 
                width = 0,
                ...
                size = 1.5, 
                colour = "black") +
  geom_text(data = full_int_plot_data,
            aes(x = d_intercept, y = 1.25, label = "Full Data"), 
            size = 4, colour = "black", 
            vjust = -2.5) +
